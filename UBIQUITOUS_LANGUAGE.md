...
| **Site Chat**          | An inline chat panel on a Site's management page (`/article/list/:siteSlug`), letting its Owner and accepted Contributors talk about the Site's Articles — backed by Bluesky's own `chat.bsky.convo` lexicon, not a CMS-native messaging system (ADR 0016/0025). Always resolved fresh against the *current* roster; never chains conversations from before a roster change. Renamed from an earlier, never-formally-defined "Team Chat" — there is no **Team** entity in this glossary, only **Owner** and **Contributor**. | Team Chat, Group chat, Messaging |
