...
| **Submission**         | A Contributor's Article offered for inclusion in someone else's Site, awaiting the Owner's **Approve** or **Reject** decision. Marked by `scribe.pendingPublish` on the Contributor's own (still-Loose) document; indexed locally in `pending_submissions` (ADR 0015) since neither party's PDS alone can answer "what's awaiting review across all my Sites/Articles." Distinct from an **Invitation** — a Submission concerns one Article, not roster membership. | Pending article, Draft submission, Pitch |
| **Site Chat**          | An inline chat panel on a Site's management page (`/article/list/:siteSlug`), letting its Owner and accepted Contributors talk about the Site's Articles — backed by a persistent `chat.bsky.group` conversation (membership synced from the Site's roster via `addMembers`/`removeMembers`), with messages read/sent via `chat.bsky.convo.getMessages`/`sendMessage` (ADR 0016/0025/0026). Created the moment the first Contributor is accepted, not before — not shown at all to an Owner with no accepted Contributors yet. Renamed from an earlier, never-formally-defined "Team Chat" — there is no **Team** entity in this glossary, only **Owner** and **Contributor**. | Team Chat, Group chat, Messaging |
...
| **User Image Folder**     | The top-level folder in the Image Library automatically created on a user's first upload. Contains all images and subfolders owned by that user. Visible and usable *only* by the owning user (ADR 0028) — read and write are both owner-only.                                                                                                        | User folder, personal folder, user directory                               |
| **Site Image Folder**     | A top-level Image Library folder owned by a Site rather than a user (ADR 0017/0020, Phase 2 of Contributors) — named `"{domain} Images"`. Created automatically alongside the Site itself. Visible and usable *only* by that Site's Owner and its accepted Contributors — same owner-or-roster-member restriction as a User Image Folder, just with a roster of more than one person (ADR 0028). | Shared folder, site folder, contributor folder                             |
...
- Every Site has exactly one **Site Image Folder**, created automatically when the Site is created. Unlike a User Image Folder, it is invisible and inaccessible to anyone except that Site's Owner and accepted Contributors — not just restricted for write, restricted for read too.
