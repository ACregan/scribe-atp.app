...
**Found live 2026-07-18 — Contributor site discoverability fix:** a Contributor's read-only access to `/article/list/:siteSlug` (Phase 5's own read-only-access fix, plus Site Chat) had no *link* to it anywhere in the CMS — a Contributor had to already know the exact URL. `listContributorSiteCards` (`contributorRoster.server.ts`) now resolves every accepted Contributor site into a full `SiteCard`-shaped entry (rkey/cid/images/counts/per-group breakdown, plus `isContributor`/`ownerDisplayName`), surfaced alongside the caller's own sites on the **Dashboard** Sites column, **Sites** page (both List and Tile views), and **Groups** page — each tagged with a "Contributor" `Pill` and with owner-only actions (Delete, Configure) hidden; Manage/Manage Articles stays, since that's the actual fix. Groups page keeps Contributor sites out of the "Add New Group" site picker (passed the owner-only `sites` array, not the combined list) — a Contributor can't create a group on a site they don't own. Dashboard's engagement charts are also computed from the caller's own sites only, not the combined list — those queries are scoped to the caller's own PDS records and would resolve to the wrong owner's data otherwise.
...
- **Pre-existing sites** (created before this at-creation-time call existed) were backfilled 2026-07-20 via a genuine one-shot script, `scripts/backfill-site-image-folders.ts` — restored an `Agent` for every DID with a stored `oauth_session` row and called the same folder-ensure logic directly, no live login needed. Deleted after running against production, per this repo's "chore: remove devtools/repair-*" convention (a bug surfaced during the run: it listed every `site.standard.publication` record on a DID with no `scribe != null` filter, unlike `sites.tsx`, so it also created a folder for one unrelated non-Scribe record on that collection — cleaned up manually; not worth fixing in a script that's now deleted). The "Resync Image Folder" button on `/site/:siteName/configure` still exists as an on-demand manual recovery path, but is no longer the backfill mechanism itself.
...
**Status: shipped, then redesigned same-day.** ADR 0025's original mechanism (per-request `getConvoForMembers`) went live, was tested between two real accounts, and turned out to only support a single Contributor — `getConvoForMembers` is 1-1 only. The actual, always-intended requirement is 1 Owner + up to ~20 Contributors, so ADR 0026 replaced the resolution mechanism with a persistent `chat.bsky.group` conversation. Everything below reflects the current (ADR 0026) state; see ADR 0025 for what's unchanged (layout, polling, pagination, send-failure UX) and ADR 0026 for what changed (resolution, error types, group membership sync).

**Depends on:** Phase 1 (roster) and, for the group itself, at least one accepted Contributor — the group is created the moment the first Contributor is accepted, not before, and not lazily on chat visit. Independent of Phases 2–4; genuinely optional relative to the rest of the feature — the submission workflow (Phase 3) functions completely without this.

**Grilled 2026-07-16 — see ADR 0025 for the original implementation decisions and ADR 0026 (2026-07-17) for the group-conversation redesign; ADR 0016 remains the high-level design record (build inline, one roster per site).**
...
- **Layout:** `/article/list/:siteSlug`'s current single scrolling `PageSection` becomes a two-column `PageSectionColumns` split — existing content (title, Groups, Submissions, Contributors) at `span={8}`, Site Chat at `span={4}` — both `overflow` — but only once the Owner has at least one accepted Contributor; with none, the page stays single-column (`span={12}`) and Site Chat isn't rendered at all (ADR 0026 Decision 5).
- **Group creation/membership sync** (ADR 0026) happens out-of-band, triggered from `reconcileContributorStatuses` (Owner-side promotion) and `removeContributor`/rejected-row reconciliation — not from the chat panel itself. `chat.bsky.group.createGroup` (first Contributor accepted) persists a `convo_id` locally (`site_chat_convos` table, since `createGroup` is non-idempotent); subsequent accepts/removals call `addMembers`/`removeMembers` against that same `convoId`.
- **Chat resolution** is now a local lookup by site identity (`ownerDid` + `:siteSlug`), not a per-request `getConvoForMembers` call — the group either already exists (created out-of-band above) or doesn't. A newly-added Contributor's own session transparently calls `acceptConvo` on first lookup if their group membership is still in "request" status — no manual accept step.
- Polling every 10s while mounted and the tab is visible, paused when backgrounded with an immediate refetch on refocus; a poll is a full re-fetch of the newest page (`limit: 50`, no cursor) diffed by message `id`, since `getMessages` has no incremental/since parameter. Unchanged by ADR 0026.
- **No pagination/"load more"** — a single page is the entire history shown. ADR 0016's original fragmentation justification for this no longer applies (a persistent group's history doesn't fragment), but 50 messages remains a reasonable single-page cap regardless.
...
- **Resolution failure states are now `notCreatedYet` (no group exists for this site yet) and `unknown`** (ADR 0026) — the four `getConvoForMembers`-specific social-graph error types (`blocked`/`messagesDisabled`/`notFollowed`/`accountSuspended`) no longer apply, since group membership is established once at accept time, not renegotiated per viewer lookup.
...
- **Explicitly out of scope:** unread badges/`updateRead` (the panel's persistent visibility makes Phase 4's "you might not notice" problem not apply here); detecting/messaging the guaranteed-on-first-run insufficient-`getMessages`-scope state; surfacing group-specific failures (`UserForbidsGroups`/`MemberLimitReached`/`InsufficientRole`/`ConvoLocked`) to the Owner — all deferred until there's a real user base beyond the developer's own two accounts (see ADR 0025/0026 Consequences).
- **Not yet done:** live verification of group creation/membership sync between two real Bluesky accounts with a genuine 2+-Contributor roster (unlike the original 1-1 mechanism, which was live-tested) — verified so far via typecheck, unit tests, and the full e2e suite only.

**Reference:** ADR 0016 (high-level design, still current for build-vs-link-out and one-roster-per-site), ADR 0025 (layout/polling/pagination/send-failure UX, still current), and ADR 0026 (group-conversation redesign — read this one for resolution, membership sync, and error handling as they currently work).
