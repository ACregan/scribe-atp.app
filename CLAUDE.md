...
| Code / Link   | Inline code `</>`, Link 🔗 (shows URL input inline when inserting), Image 🖼 (shows URL input inline when inserting) |
...
- **Image insertion — `imageNode.tsx`** (`app/components/RichTextEditor/imageNode.tsx`): `ImageNode extends DecoratorNode` with `INSERT_IMAGE_COMMAND`. `importDOM` handles `<img>` tags on load; `exportDOM` creates an `<img>` element on serialisation. Registered in `EDITOR_NODES`. The toolbar's 🖼 button dispatches `INSERT_IMAGE_COMMAND` with `{ src, altText }`.
