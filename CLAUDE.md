...
/article/list/:siteSlug        site-list       — site-scoped article/group management; reads/writes site.standard.publication
...
All articles live in `site.standard.document`. Unpublished (in-progress) articles are referenced in a publication's `ungroupedArticles`. Published articles are referenced in a publication's `groups[].articles`. There is no separate draft collection — the distinction is purely about which list inside the publication record an article ref appears in.
...
The `/article/list/:siteSlug` route is the site-scoped management view. It reads the site record, builds a DnD tree, and writes the updated site record back. Actions: `createGroup`, `deleteGroup`, `saveSite`, `removeArticle`, `publishArticle`, `moveToDraft`. **Remove article only removes it from the site record — it does not delete the PDS article record.**
...
- **Publish/Draft workflow** — the "Unpublished Articles" root group (formerly "Ungrouped Articles") shows articles not yet in any named group. Each article shows a **Publish** button (success variant) that opens a group-picker modal; submits `_intent=publishArticle` with `uri` and `targetGroupSlug`, which moves the article ref from `ungroupedArticles` into the chosen group. Articles in named groups show a **Move to Drafts** button that opens a confirmation modal naming the source site and group; submits `_intent=moveToDraft` with `uri` and `sourceGroupSlug`, which moves the ref back to `ungroupedArticles`.
...
| `buildArticleRecord(fields)`                    | Constructs the `site.standard.document` PDS record object from article fields                                                       |
...
| `loadSiteOptions(agent, did)`                   | Fetches all `site.standard.publication` records and returns `SiteOption[]` for the multi-select                                     |
