...
## Status
Accepted and implemented — design finalized 2026-07-16 (Phase 3 sub-pass 3 grill session). The Standalone-vs-Site-Assigned misclassification noted below under Consequences (found live during the Phase 3c test pass) has since been fixed — `list.tsx` now resolves the external site so these documents correctly show under Site-Assigned Articles.
...
- **Found while implementing, backlogged rather than fixed here — since fixed (2026-07-16, live Phase 3c test pass):** `list.tsx`'s Standalone-vs-Site-Assigned classification keyed off `assignmentMap.has(uri)`, built only from the caller's *own* sites (`listSites`). Once this ADR's finalizing write publishes a Contributor's document to the **Owner's** site (a different account), the document was genuinely published but never appeared in `assignmentMap` — so it kept showing under Standalone Articles with a live Publish button again. Fix: `list.tsx` now resolves any document whose `value.site` is an `at://` URI not already covered by `assignmentMap` — the same public cross-repo site-record read `reconcilePendingSubmission` already does (`getPublicSiteRecord`, now exported from `submissionReview.server.ts`) — and merges the result into the same `assignmentMap` before classification. The Unpublish button is additionally gated to only show when the caller owns the resolved site, since unpublishing writes to the site's own manifest via the caller's session and a Contributor has no valid way to do that for someone else's site.
