...
- **Found while implementing, backlogged rather than fixed here:** `list.tsx`'s Standalone-vs-Site-Assigned classification keys off `assignmentMap.has(uri)`, built only from the caller's *own* sites (`listSites`). Once this ADR's finalizing write publishes a Contributor's document to the **Owner's** site (a different account), the document is genuinely published but never appears in `assignmentMap` — so it keeps showing under Standalone Articles, minus the "Pending Review" pill, with a live Publish button again. Harmless (the submit guard from ADR 0021 point 5 already rejects re-submitting an already-`at://` document), but misleading. Explicitly deferred to Phase 4's UX polish rather than fixed in this sub-pass, which was scoped to write mechanics, not display — same posture as ADR 0020's folder-deletion gap and ADR 0015's lost-index gap.
