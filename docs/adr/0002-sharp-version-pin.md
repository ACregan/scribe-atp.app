# ADR 0002: Pin sharp to 0.32.x for VPS CPU Compatibility

## Status
Accepted

## Context
Sharp 0.33.0 changed its prebuilt binary distribution to require the x86_64-v2 microarchitecture (SSE4.2 and related instruction sets). The production VPS runs an older x64 CPU that does not meet this requirement (`grep -m1 flags /proc/cpuinfo | grep -o sse4_2` returns no output).

Attempting to run sharp 0.33+ on this VPS produces:
...
## Consequences
- Do not upgrade sharp past `0.32.x` without first checking the production CPU: `grep -m1 flags /proc/cpuinfo | grep -o sse4_2`. A non-empty result means x86_64-v2 is supported and the pin can be relaxed.
- The sharp 0.32.x API covers all operations used by the Image Service (`metadata`, `resize`, `webp`, `toFile`) with no functional regression.
