...
import { browseImages, ensureSiteFolder } from "./imageServiceClient.server";
...
describe("ensureSiteFolder", () => {
  const SITE_URI = "at://did:plc:owner/site.standard.publication/my-site";

  it("PUTs the site folder request with the correct body and cookie forwarding", async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal("fetch", fetchMock);

    await ensureSiteFolder(SITE_URI, "example.com", "__session=abc");

    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:3009/api/image-service/site-folder",
  ...
  it("throws when the Image Service responds with a non-OK status", async () => {
    ...
    await expect(ensureSiteFolder(SITE_URI, "example.com", "__session=abc")).rejects.toThrow("403");
  });
});
